#include "COMETS3357/pathplanner/lib/auto/BaseAutoBuilder.h"
#include "COMETS3357/pathplanner/lib/commands/FollowPathWithEvents.h"
...
frc2::CommandPtr BaseAutoBuilder::followPathGroup(std::vector<PathPlannerTrajectory> pathGroup) 
{
...
frc2::CommandPtr BaseAutoBuilder::followPathWithEvents(PathPlannerTrajectory trajectory) 
{
	return FollowPathWithEvents(followPath(trajectory).Unwrap(),trajectory.getMarkers(), m_eventMap).ToPtr();
}

frc2::CommandPtr BaseAutoBuilder::followPathGroupWithEvents(std::vector<PathPlannerTrajectory> pathGroup) 
{
	std::vector < frc2::CommandPtr > commands;
...
frc2::CommandPtr BaseAutoBuilder::resetPose(PathPlannerTrajectory trajectory) 
{
	if (m_drivetrainType == DriveTrainType::HOLONOMIC) 
	{
		return frc2::cmd::RunOnce([this, trajectory]() 
		{
			...
			{
				initialState = PathPlannerTrajectory::transformStateForAlliance(initialState,frc::DriverStation::GetAlliance());
			}

			m_resetPose(frc::Pose2d(initialState.pose.Translation(),initialState.holonomicRotation));
		});
	} else {
		return frc2::cmd::RunOnce([this, trajectory]() 
			{
...
frc2::CommandPtr BaseAutoBuilder::getStopEventCommands(PathPlannerTrajectory::StopEvent stopEvent) 
{
	std::vector < frc2::CommandPtr > eventCommands;
	for (size_t i = (stopEvent.executionBehavior == PathPlannerTrajectory::StopEvent::ExecutionBehavior::PARALLEL_DEADLINE ? 1 : 0); i < stopEvent.names.size(); i++) 
	{
		std::string name = stopEvent.names[i];
		if (m_eventMap.find(name) != m_eventMap.end()) 
		{
			eventCommands.emplace_back(wrappedEventCommand(m_eventMap.at(name)));
		}
	}
	...
	else if (stopEvent.executionBehavior == PathPlannerTrajectory::StopEvent::ExecutionBehavior::PARALLEL) 
	{
	...
	else if (stopEvent.executionBehavior == PathPlannerTrajectory::StopEvent::ExecutionBehavior::PARALLEL_DEADLINE) 
	{
		...
		return frc2::cmd::Deadline(std::move(deadline), std::move(eventCommands));
	}
	return frc2::cmd::None();
}

frc2::CommandPtr BaseAutoBuilder::stopEventGroup(PathPlannerTrajectory::StopEvent stopEvent) 
{
	...
	else if (stopEvent.waitBehavior == PathPlannerTrajectory::StopEvent::WaitBehavior::AFTER) 
	{
		std::vector < frc2::CommandPtr > commands;
		commands.emplace_back(std::move(events));
		commands.emplace_back(frc2::cmd::Wait(stopEvent.waitTime));
	...
	else if (stopEvent.waitBehavior == PathPlannerTrajectory::StopEvent::WaitBehavior::DEADLINE) 
	{
		std::vector < frc2::CommandPtr > commands;
		commands.emplace_back(std::move(events));
		commands.emplace_back(frc2::cmd::Wait(stopEvent.waitTime));
	...
	else if (stopEvent.waitBehavior == PathPlannerTrajectory::StopEvent::WaitBehavior::MINIMUM) 
	{
		std::vector < frc2::CommandPtr > commands;
		commands.emplace_back(frc2::cmd::Wait(stopEvent.waitTime));
		commands.emplace_back(std::move(events));
...
frc2::CommandPtr BaseAutoBuilder::fullAuto(std::vector<PathPlannerTrajectory> pathGroup) 
{
	std::vector < frc2::CommandPtr > commands;
	...
	commands.emplace_back(stopEventGroup(pathGroup[pathGroup.size() - 1].getEndStopEvent()));
